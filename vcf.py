...
#--------------------------------------------------------------------------------------
def biAllelicVar(alleles, RepRegion, outVcf, outVariants, tumorNormal):
   ID = '.'
   if tumorNormal:
...
# main function
#--------------------------------------------------------------------------------------
def makeVcf(runPath, outlong, sampleName, refGenome, tumorNormal = False):
   ...
   with open(outlong, 'r') as f:
      ...
      for line in f:
         ...
         # for new chrom or position, if last variant is not multi-allelic, write to vcf directly
            ...
            biAllelicVar(alleles, RepRegion, outVcf, outVariants, tumorNormal)
            alleles = [currentAllele]

         # if last variant is possible multi-allelic, combine and write as one 
         else:            
            multiAllelicVar(alleles, RepRegion, outVcf, outVariants, tumorNormal)
            alleles = [currentAllele]
